        ...
        <form action="{{ route('tenants.update', $tenant) }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <div class="input-group">
                        <input type="text" name="domain" class="form-control @error('domain') is-invalid @enderror" value="{{ old('domain', $tenant->domain) }}">
            ...
            <div class="row border-top pt-3 mt-2">
                <div class="col-md-4 mb-3">
                    ...
                    <select name="plan_name" class="form-select @error('plan_name') is-invalid @enderror">
                        <option value="Free" {{ old('plan_name', $tenant->plan_name) == 'Free' ? 'selected' : '' }}>Free</option>
                        <option value="Basic" {{ old('plan_name', $tenant->plan_name) == 'Basic' ? 'selected' : '' }}>Basic</option>
                        <option value="Premium" {{ old('plan_name', $tenant->plan_name) == 'Premium' ? 'selected' : '' }}>Premium</option>
                        <option value="Enterprise" {{ old('plan_name', $tenant->plan_name) == 'Enterprise' ? 'selected' : '' }}>Enterprise</option>
                    </select>
                    @error('plan_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">Trial Ends At</label>
                    <input type="date" name="trial_ends_at" class="form-control @error('trial_ends_at') is-invalid @enderror" value="{{ old('trial_ends_at', $tenant->trial_ends_at?->format('Y-m-d')) }}">
                    @error('trial_ends_at')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">Subscription Ends At</label>
                    <input type="date" name="subscription_ends_at" class="form-control @error('subscription_ends_at') is-invalid @enderror" value="{{ old('subscription_ends_at', $tenant->subscription_ends_at?->format('Y-m-d')) }}">
            ...
            <div class="mb-3 mt-2">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" name="is_active" id="isActive" value="1" {{ old('is_active', $tenant->is_active) ? 'checked' : '' }}>
